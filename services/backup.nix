  ...
  config = lib.mkIf (config.ruben.fullbackup.enable)
    {
      ...
      services.restic.backups.fullbackup = {
        ...
        paths = [ "/home/ruben" ] ++ pkgs.lib.ifEnable config.ruben.gitserver.enable [ config.ruben.gitserver.path ]
        ...
        backupPrepareCommand = "${pkgs.curl}/bin/curl -fsS -m 10 --retry 5 https://hc-ping.com/$(${pkgs.coreutils}/bin/cat ${config.age.secrets.healthchecksIoUuid.path})/${hostname}-backup/start";
        pruneOpts = [
          "--keep-hourly 48"
          "--keep-daily 7"
          "--keep-weekly 4"
